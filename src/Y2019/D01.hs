...
module Y2019.D01 (d01a, d01b) where
...
-- he needs you to bring him measurements from __fifty stars__.
--
-- Collect stars by solving puzzles. Two puzzles will be made available on each
-- day in the Advent calendar; the second puzzle is unlocked when you complete
-- the first. Each puzzle grants __one star__. Good luck!
--
...
--
-- Fuel required to launch a given __module__ is based on its __mass__.
...
--
-- __What is the sum of the fuel requirements__ for all of the modules on your
-- spacecraft?
d01a :: Text -> Natural
d01a =
  sum
    . map
      ( subtract 2
          . (`quot` 3) -- divide by 3, rounding down
          . uncurry (flip (assert . null)) -- assert no text remains
          . either error id -- report error if present
          . decimal -- parse textual digits into Natural
      )
    . lines -- each line is one module
...
-- three, round down, and subtract 2. However, that fuel __also__ requires fuel,
-- and __that__ fuel requires fuel, and so on. Any mass that would require
-- __negative fuel__ should instead be treated as if it requires __zero fuel__;
-- the remaining mass, if any, is instead handled by __wishing really hard__,
...
--      fuel (2 divided by 3 and rounded down is @0@, which would just call for
--      a negative fuel), so the total fuel required is still just @2@.
--   -  At first, a module of mass @1969@ requires @654@ fuel. Then, this fuel
--      requires @216@ more fuel (@254 / 3 - 2@). @216@ then requires @70@ more
--      fuel, which requires @21@ more fuel, which requires @5@ fuel, which
